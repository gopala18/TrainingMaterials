...
SELECT project_id,
		...
		FROM allocation 
		GROUP BY project_id
		HAVING COUNT(DISTINCT emp_id)>= all(SELECT COUNT(DISTINCT emp_id) 
													FROM allocation 
													GROUP BY project_id);
...
SELECT emp_id,emp_name FROM employee WHERE emp_id NOT IN (SELECT emp_id FROM allocation GROUP BY emp_id);
